...
import { Calendar, AlertCircle, FileText, Clock, Bell } from 'lucide-react';
						...
						{taxFilings
							...
							>
								...
								<div className="flex items-center gap-2">
									...
									<span className="text-sm font-poppins font-medium">{filing.type}</span>
								</div>
					...
					<div className="bg-white rounded-xl shadow-sm">
						...
						<div className="flex space-x-8 px-6 py-5">
							...
							className={`py-3 px-2 -mb-px text-xs font-semibold font-poppins ${
								activeTab === 'filings'
								? 'border-b-2 bg-gray-800 text-white'
								: 'text-white'
							}`}
						...
						{activeTab === 'filings' && (
							...
							onExportExcel={() => handleExportExcel(taxFilings, 'tax-filings')}
							onExportPDF={() =>
								handleExportPDF(
								taxFilings,
								filingColumns.map(col => ({
						...
						{activeTab === 'vat' && (
							...
							data={vatEntries}
							columns={vatColumns}
							onExportExcel={() => handleExportExcel(vatEntries, 'vat-entries')}
							onExportPDF={() =>
								handleExportPDF(
								vatEntries,
								vatColumns.map(col => ({
						...
						{activeTab === 'income' && (
							...
							onExportExcel={() =>
								handleExportExcel(incomeTaxEntries, 'income-tax')
							}
							onExportPDF={() =>
								handleExportPDF(
								incomeTaxEntries,
								incomeTaxColumns.map(col => ({
									header: col.header as string,
									accessor: col.accessorKey as string,
								})),
								'income-tax',
